...
$stmt = db()->prepare('SELECT * FROM bands WHERE school_id = ? AND active = 1 ORDER BY band_key');
$stmt->execute([$schoolId]);
$bands = $stmt->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'create') {
    ...
    if ($name === '' || $bandId === 0) {
        ...
        try {
            $stmt = db()->prepare(
                'INSERT INTO classes (school_id, band_id, name, room_id, student_count, active) VALUES (?, ?, ?, ?, ?, 1)'
            );
            $stmt->execute([
                $schoolId,
                $bandId,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'toggle') {
    ...
    try {
        ...
        $stmt->execute([$active, $classId, $schoolId]);
