...
<p class="empty">
    Drag-and-drop is not available in this simple grid — instead, use the dropdown next to each
    lesson to move it. Every move is checked against the current timetable before it's saved; a
    move that would double-book a teacher, room, or class is rejected with a specific reason.
    <strong>Regenerating the whole-school timetable will discard these manual changes</strong> —
    they apply only to the current run shown here.
</p>
...
<?php else: ?>
    ...
    <?php if ($classId !== 0): ?>
        ...
        <div class="card">
            <h2>Lessons for this class</h2>
            <?php if (empty($slots)): ?>
                <p class="empty">No scheduled lessons found for this class in the latest generation.</p>
            <?php else: ?>
                <table>
                    <thead><tr><th>Subject/Activity</th><th>Teacher</th><th>Room</th><th>Day</th><th>Time</th><th>Move to</th></tr></thead>
                    <tbody>
                        <?php foreach ($slots as $s): ?>
                            <tr id="slot-row-<?php echo (int) $s['id']; ?>">
                                <td><?php echo htmlspecialchars($s['subject_name'] ?? $s['activity_name'] ?? '—'); ?>
